...
class TestGetStatistic:
    ...
    @allure.description(
        "Проверка соответствия данных в сервисе статистики после создания объявления"
    )
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.parametrize("api_version", ["api/1", "api/2"])
    @pytest.mark.asyncio(loop_scope="session")
    async def test_statistic_consistency_v1_v2(
        self, api_client, api_version, item_payload_factory
    ):
        with allure.step("Подготовка тестовых данных"):
            target_stats = Statistics(likes=777, view_count=1000, contacts=50)
            payload = item_payload_factory(
                name=f"Stat Check {api_version}", price=100000, statistics=target_stats
            )
    ...
    async def test_statistic_not_found_v1_v2(self, api_client, api_version, fake_uuid):
        ...
        with allure.step(
            "Проверка наличия полей result и status согласно документации"
        ):
    ...
    @pytest.mark.parametrize("api_version", ["api/1", "api/2"])
    @pytest.mark.asyncio(loop_scope="session")
    async def test_statistic_idempotency_v1_v2(
        self, api_client, created_item_id, api_version
    ):
    ...
    @pytest.mark.asyncio(loop_scope="session")
    async def test_get_item_statistic_invalid_formats(
        self, api_client, invalid_path, expected_codes, created_item_id
    ):
        # Логика для формирования URL (обработка спецкейса со слешем)
        path = f"/api/2/statistic/{invalid_path}"
        if "slash" in path:
            path = f"/api/2/statistic//{created_item_id}"
