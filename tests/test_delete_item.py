...
class TestDeleteItem:
    ...
    @allure.description("Проверка смены статуса ресурса с 200 на 404 после выполнения DELETE")
    ...
    async def test_delete_item_e2e(self, api_client, created_item_id):
        with allure.step(f"Выполнение запроса DELETE для ID: {created_item_id}"):
            ...
            allure.attach(str(delete_res.status_code), name="Delete Status Code", attachment_type=allure.attachment_type.TEXT)
            
            # Проверка допустимых кодов (с учетом бага 405, если он есть)
        ...
        with allure.step("Проверка отсутствия объявления через GET"):
            ...
            allure.attach(str(get_res.status_code), name="GET Status After Delete", attachment_type=allure.attachment_type.TEXT)
            
            assert get_res.status_code == 404, (
                f"BUG-05: Объявление {created_item_id} все еще доступно (Status {get_res.status_code})"
            )
    ...
    async def test_delete_non_existent_item(self, api_client, fake_uuid):
        with allure.step(f"Попытка удаления фейкового UUID: {fake_uuid}"):
    ...
    @allure.description("Проверка идемпотентности/корректности обработки повторных DELETE запросов")
    ...
    @allure.severity(allure.severity_level.MINOR)
    @pytest.mark.parametrize("invalid_id, test_name", [
    ...
    async def test_delete_invalid_ids(self, api_client, invalid_id, test_name):
        with allure.step(f"Удаление ID с нарушением формата: {test_name} ('{invalid_id}')"):
