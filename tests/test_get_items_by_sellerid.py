...
from models.models import ItemResponse, ItemRequest
...
async def test_get_items_by_seller_id_full_check(api_client):
    ...
    for item in items:
        assert item.sellerId == seller_id, f"Найдено чужое объявление! SellerId: {item.sellerId}"
...
async def test_get_items_empty_seller(api_client):
    ...
    assert response.status_code == 200, f"Ожидали 200 для пустого списка, получили {response.status_code}"
    data = response.json()
    assert isinstance(data, list), "Ответ должен быть списком (массивом)"
    assert len(data) == 0, f"Список должен быть пустым, но найдено {len(data)} элементов"
...
async def test_get_items_invalid_seller_id_type(api_client):
    ...
    assert response.status_code == 400, f"Ожидали 400 для невалидного ID, получили {response.status_code}"
...
async def test_get_items_negative_seller_id(api_client):
    ...
    assert response.status_code in [400, 404], f"Неправильный статус для отрицательного ID: {response.status_code}"
...
async def test_get_items_large_seller_id(api_client):
    ...
    assert response.status_code != 500, "Сервер упал с 500 ошибкой при слишком большом sellerId"
