...
3. Через командную строку/терминал перейдите в корневую директорию проекта:
   ...
   ```
4. Установите все необходимые зависимости из файла requirements.txt. Это обязательный шаг для корректной работы всех библиотек (pytest, allure-pytest, httpx):
    ```bash
    ...
    ```
5. Для выполнения автотестов выполните команду:
    ```bash
    pytest
    ```
...
6. Для генерации и просмотра отчетов убедитесь, что в Вашей системе установлен Allure Commandline.
...
7. Наконец, запустите тесты и сгенерируйте отчет. Для этого необходимо выполнить две команды последовательно:Шаг А: Запуск тестов и сбор результатов в папку allure-results:
    ```bash
...
8. Завершить просмотр документации можно выполнив команду:
    ```bash
    Ctrl + C
    ```
    с параметром Y.